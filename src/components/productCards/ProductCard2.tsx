...
const ProductCard2 = React.memo(({ product }: ProductCardProps) => {
  const {
    addProductToCart,
  ...
  const isInWishlist = useMemo(
    ...
    [product.id, isAddedtoWishlist]
  );
  const isInCompare = useMemo(
    () => isAddedtoCompareItem(product.id),
    [product.id, isAddedtoCompareItem]
  );
      ...
      <div className="card-product-wrapper">
        <Link to={`/product-detail/${product.id}`} className="image-wrap">
          <img
            ...
            alt={product.title}
            src={currentItem || product.imgSrc}
            width={900}
            height={900}
          />
          <img

...
import { twoDrawerCabinetMain } from "@/data/TwoDrawerCabinet"; 
 
// Filter state type (update this to match your reducer state exactly)   
 
// Props for Products1 
interface TwoDrawerCabinetProps {
  parentClass?: string;
...
export default function twoDrawerCabinet({
  parentClass = "flat-spacing", 
  defaultActiveLayout = 4, 
  isFullLayout = false, 
  cardStyle = 1, 
}: TwoDrawerCabinetProps) {
  ...
  useEffect(() => {
     dispatch({ type: "FILTER_PRODUCTS", payload: twoDrawerCabinetMain });
  }, [ 
    price,
    availability,
    color,

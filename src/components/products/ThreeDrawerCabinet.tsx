...
import { threeDrawerCabinetMain } from "@/data/ThreeDrawer"; 
 
// Filter state type (update this to match your reducer state exactly)   

// Props for Products1 
interface ThreeDrawerCabinetProps {
  parentClass?: string;
...
export default function threeDrawerCabinet({
  parentClass = "flat-spacing", 
  defaultActiveLayout = 4, 
  isFullLayout = false, 
  cardStyle = 1, 
}: ThreeDrawerCabinetProps) {
  ...
  useEffect(() => {
     dispatch({ type: "FILTER_PRODUCTS", payload: threeDrawerCabinetMain });
  }, [ 
    price,
    availability,
    color,

...
import { diningEightSeaterMain } from "@/data/diningEightSeater"; 
 
// Filter state type (update this to match your reducer state exactly) 
 
// Props for Products1
interface DiningEightSeaterMainProps { 
  parentClass?: string;
...
export default function DiningEightSeater({
  parentClass = "flat-spacing", 
  defaultActiveLayout = 4, 
  isFullLayout = false, 
  cardStyle = 1, 
}: DiningEightSeaterMainProps) { 
  ...
  useEffect(() => {
     dispatch({ type: "FILTER_PRODUCTS", payload: diningEightSeaterMain });
  }, [ 
    price,
    availability,
    color,

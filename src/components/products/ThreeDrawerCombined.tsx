...
import { threeDrawerCombinedCabinetMain } from "@/data/ThreeDrawerCombined"; 
 
// Filter state type (update this to match your reducer state exactly)   
 
// Props for Products1 
interface TwoDrawerCombinedCabinetProps {
  parentClass?: string;
...
export default function threeDrawerCombinedCabinet({
  parentClass = "flat-spacing", 
  defaultActiveLayout = 4, 
  isFullLayout = false, 
  cardStyle = 1, 
}: TwoDrawerCombinedCabinetProps) {
  ...
  useEffect(() => {
     dispatch({ type: "FILTER_PRODUCTS", payload: threeDrawerCombinedCabinetMain }); 
  }, [ 
    price,
    availability,
    color,

...
import { mediumBackChairMain } from "@/data/MediumBackChair"; 
 
// Filter state type (update this to match your reducer state exactly)   
 
// Props for Products1
interface MediumBackChairProps {
  parentClass?: string;
...
export default function mediumBackChair({
  parentClass = "flat-spacing", 
  defaultActiveLayout = 4, 
  isFullLayout = false, 
  cardStyle = 1, 
}: MediumBackChairProps) {
  ...
  useEffect(() => {
     dispatch({ type: "FILTER_PRODUCTS", payload: mediumBackChairMain });
  }, [ 
    price,
    availability,
    color,

...
import { diningFourSeaterMain } from "@/data/diningFourseater"; 
 
// Filter state type (update this to match your reducer state exactly) 
 
// Props for Products1
interface DiningFourSeaterProps {
  parentClass?: string;
...
export default function DiningFourSeater({
  parentClass = "flat-spacing", 
  defaultActiveLayout = 4, 
  isFullLayout = false, 
  cardStyle = 1, 
}: DiningFourSeaterProps) { 
  ...
  useEffect(() => {
     dispatch({ type: "FILTER_PRODUCTS", payload: diningFourSeaterMain });
  }, [ 
    price,
    availability,
    color,

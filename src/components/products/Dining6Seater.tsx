...
import { diningSixSeaterMain } from "@/data/diningSixSeater"; 
 
// Filter state type (update this to match your reducer state exactly) 
 
// Props for Products1
interface DiningSixSeaterProps { 
  parentClass?: string;
...
export default function DiningSixSeater({
  parentClass = "flat-spacing", 
  defaultActiveLayout = 4, 
  isFullLayout = false, 
  cardStyle = 1, 
}: DiningSixSeaterProps) { 
  ...
  useEffect(() => {
     dispatch({ type: "FILTER_PRODUCTS", payload: diningSixSeaterMain });
  }, [ 
    price,
    availability,
    color,
